...
{
	private static final String KICK_REASON = "Vous avez été banni du serveur par %1$s :\n\n%2$s";
	private static final String BAN_MESSAGE = "Le joueur {%1$s} a été banni d'Heavencraft.";
	private static final String UNBAN_MESSAGE = "Le joueur {%1$s} a été débanni d'Heavencraft.";

	private static final String GET_REASON_QUERY = "SELECT CONCAT_WS(' : ', banned_by, reason) FROM banlist WHERE uuid = ?;";
	...
	public static String getReason(UUID uniqueId)
	{
		try (Connection connection = HeavenProxyInstance.get().getConnectionProvider().getConnection();
				PreparedStatement ps = connection.prepareStatement(GET_REASON_QUERY))
		{
